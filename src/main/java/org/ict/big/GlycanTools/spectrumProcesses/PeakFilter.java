...
//                ratio = Math.max(ratio, p.getRelativeIntensity());
    ...
    public PeakList filter(PeakList original){
        ...
        for(Peak p: original.getPeaks()){
            if(p.getRelativeIntensity() < 0){
                p.setRelativeIntensity(p.getRelativeIntensity() / original.getMaxIntensity() * 100.);
            }
            if(p.getRelativeIntensity() >= filterRatio * 100.){
                pl.addPeak(p.clone());
            }
        }
        return pl;
    ...
    public void filterInPlace(PeakList peakList){
        ...
        for(Peak p: peakList.getPeaks()){
            if(p.getRelativeIntensity() < 0){
                p.setRelativeIntensity(p.getAbsoluteIntensity() / peakList.getMaxIntensity() * 100.);
            }
            if(p.getRelativeIntensity() >= filterRatio * 100.){
                peaks.add(p.clone());
            }
